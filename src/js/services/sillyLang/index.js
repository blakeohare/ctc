(() => {
                ...
                case '.':
                    {
                        ...
                        expr = { type: 'field', root: expr, fieldName };
                    }
                    break;
    ...
    const runExpression = (expr, vars) => {
        ...
        switch (expr.type) {
            ...
            case 'ops':
                {
                    let expressions = expr.expressions.map(t => runExpression(t, vars));
                    ...
                    for (let i = 0; i < expr.ops.length; i++) {
                        ...
                        switch (signature) {
                            ...
                            case 'int<int': left = { type: 'bool', value: left.value < right.value }; break;
                            case 'int>int': left = { type: 'bool', value: left.value > right.value }; break;
                            case 'int<=int': left = { type: 'bool', value: left.value <= right.value }; break;
                            case 'int>=int': left = { type: 'bool', value: left.value >= right.value }; break;
        ...
        while (pc < len) {
            ...
            if (line) {
                switch (line.type) {
                    case 'assign':
                        {
                            ...
                            let exprValue = runExpression(value, variables);
                    ...
                    case 'if':
                        {
                            ...
                            let exprValue = runExpression(condition, variables);
                    ...
                    case 'print':
                        {
                            let value = runExpression(line.expression, variables);
